...
// Comparator function
template <typename T>
std::vector<size_t> sort_indices(const std::vector<T>& data) {
   size_t n = data.size();
   ...
   // Reverse the order of indices for descending order
...
//vector<int> v = {1, 3, 2, 4, 5, 0};vector<size_t> idx = sort_indices_desc(v); for (size_t i : idx) {std::cout << i << " ";}
...
void solve() {
   ...
   while (t--) {
      int n;
      cin >> n;
      vector<int> a(n, 0), b(n, 0), vec(n, 0);
      for (int i = 0; i < n; i++) cin >> a[i];
      for (int i = 0; i < n; i++) cin >> b[i];
      vector<size_t> idx_a = sort_indices(a);
      reverse(all(idx_a));
      for (auto ele : idx_a) cout << ele << endl;
   }
