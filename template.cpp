...
private:
   ...
   std::vector<T> vec;
   int n;
   std::function<T(T, T)> mergeOp;

   // Helper function to query a range
   T query(int ind, int low, int high, int l, int r) {
      if (low > r || high < l) return std::numeric_limits<T>::max(); // Adjust for non-numeric types if needed
      if (low >= l && high <= r) return seg[ind];
      int mid = low + (high - low) / 2;
      return mergeOp(query(2 * ind + 1, low, mid, l, r), query(2 * ind + 2, mid + 1, high, l, r));
   }

   // Helper function to update a single element
   void update(int ind, int low, int high, int idx, T val) {
      if (low == high) {
         seg[ind] = val;
         vec[idx] = val; // Update original vector as well
         return;
      }
      int mid = low + (high - low) / 2;
      if (idx <= mid) update(2 * ind + 1, low, mid, idx, val);
      else update(2 * ind + 2, mid + 1, high, idx, val);
      seg[ind] = mergeOp(seg[2 * ind + 1], seg[2 * ind + 2]);
   }

public:
   SGT(const std::vector<T> &inputVec, std::function<T(T, T)> op) : vec(inputVec), n(inputVec.size()), mergeOp(op) {
      seg.resize(4 * n + 1);
      build(0, 0, n - 1);
   }
...
//reverse(all(vec))
//cout<< [](int x){ return x+2;} (4)<<endl; //ans =6
//auto sum=[](int x,int y){return x+y;};
//int x=sum(2,3); x=5;
//vector<int> vec{1,2,3};
//cout<<all_of(all(v),[](int x){return x>0;}); // if all of the element is greater than zero
//cout<<any_of(all(v),[](int x){return x>0;}); // if all of the element is greater than zero
//cout<<none_of(all(v),[](int x){return x>0;}); // if all of the element is greater than zero
//int d=5,e=3;
//for_each(all(vec),[&d,&e](int x){cout<<x+d+e<<endl;}); //[&] all variable are passed to capture class can be changed also, [=] all variable passed but cannot be changed
template <typename T>
vector<size_t> argsort(const vector<T>& arr,
   ...
   [&](size_t i1, size_t i2) { return comparator(arr[i1], arr[i2]); });
   return indices;
}
// vector<size_t> descending_indices = argsort(arr, [](const int& a, const int& b) {
//    return a > b;  // Reverse the comparison for descending order
// });
template <typename T>
vector<T> get_sorted_array(const vector<T>& arr, const vector<size_t>& indices) {
   vector<T> sorted_arr(arr.size());
...
template <typename T>
auto enumerate(T&& container) -> std::vector<std::pair<size_t, decltype(*std::begin(container))>> {
   std::vector<std::pair<size_t, decltype(*std::begin(container))>> result;
...
//          std::cout << "Person " << index << ": " << name << std::endl;
//       }
