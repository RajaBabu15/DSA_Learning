...
bool compareFunc(const vector<ll>& a, const vector<ll>& b) {
   if (a[1] == b[1]) {
      return a[0] < b[0]; // If second elements are equal, compare the first elements
   } else {
...
void solve() {
   ...
   make_heap(vec.begin(), vec.end(), compareFunc);
   for (const auto& element : vec) {
      cout << element[0] << " (from sub-vector " << element[1] << ", element " << element[2] << ")" << endl;
   }
