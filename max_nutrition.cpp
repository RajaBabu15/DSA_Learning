...
//accumulate(arr,arr+n,sum(0ll),myfun) //sum all the element in the arr
...
void solve() {
   ...
   while (t--) {
      int n;
      cin >> n;
      vector<int> type(n, 0), nutrition(n, 0);
      for (auto &ele : type) cin >> ele;
      for (auto &ele : nutrition) cin >> ele;
      vector<int> vec(n + 1, 0);
      for (int i = 0; i < n; i++) {
         vec[type[i]] = max(vec[type[i]], nutrition[i]);
      }
      cout << accumulate(vec.begin(), vec.begin() + vec.size(), 0ll) << endl;
   }
}
