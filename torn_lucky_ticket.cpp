...
void solve() {
	...
	while (t--) {
		int n;
		cin >> n;
		vector<pair<string , int >> vecE, vecO;
		string temp;
		int len, count;
		for (int i = 0; i < n; i++) {
			cin >> temp;
			...
			if (len & 1)  vecO.pb(mp(temp, count));

			else vecE.pb(mp(temp, count));
		}
		int ans = 0;
		for (int i = 0; i < vecE.size(); i++) {
			for (int j = 0; j < vecE.size(); j++) {
				if (vecE[i].ff.length() == vecE[j].ff.length() && vecE[i].ss == vecE[j].ss) {
					if (i != j) ans += 2;
					else ans += 1;
				}
				else {
					string f, s;
					if (vecE[i].ff.length() == 2) { f = vecE[i].ff; s = vecE[j].ss;}
					else { f = vecE[j].ff; s = vecE[i].ss;}
					int var = abs(vecE[i].ss - vecE[j].ss);
					if (var == 2 * (s[0] - '0') || var == 2 * (s[s.length() - 1] - '0')) {
						if (i == j) ans += 1;
						else ans += 2;
					}
				}
