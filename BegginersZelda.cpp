...
vector<int> adj[100001], diameter;
int visited[100001], parent[100001];
int maxD = -1, maxNode = -1, sum = 0;
...
void createDiameter(int node) {
   diameter.push_back(node);
   sum += node;
   if (node != maxNode) createDiameter(parent[node]);
}
...
void solve() {
   ...
   while (t--) {
      int n, u, v;
      cin >> n;
      vector<int> vec(n + 1, 0); // Increase the size of the vector by 1
      for (int i = 1; i < n; i++) {
         cin >> u >> v;
         vec[u]++;
         vec[v]++;
