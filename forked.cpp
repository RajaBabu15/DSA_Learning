...
void possible(int x, int y, int xq, int yq, int a, int b, set<pair<int, int>> &st) { //x,y =possition of the knight attacking the king for sure
   int dx = abs(xq - x);
   int dy = abs(yq - y);
...
void solve() {
   ...
   while (t--) {
      ...
      set<pair<int, int>> st;
      cin >> a >> b >> xk >> yk >> xq >> yq;
      possible(xk + a, yk + b, xq, yq, a, b, st);
      possible(xk + a, yk - b, xq, yq, a, b, st);
      possible(xk - a, yk + b, xq, yq, a, b, st);
      possible(xk - a, yk - b, xq, yq, a, b, st);

      possible(xk + b, yk + a, xq, yq, a, b, st);
      possible(xk + b, yk - a, xq, yq, a, b, st);
      possible(xk - b, yk + a, xq, yq, a, b, st);
      possible(xk - b, yk - a, xq, yq, a, b, st);
      cout << st.size() << endl;
